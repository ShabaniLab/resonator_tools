...
class notch_port(circlefit, save_load, plotting, calibration):
        ...
        if calc_errors==True:
            ...
            if cov is not None:
                ...
                errors = {
                    ...
                    "prefactor_a_err": 0, # Temporary until error calculation can be introduced
                    "prefactor_alpha_err": 0  # Temporary until error calculation can be introduced
                }
                results.update(errors)
            else:
                print("WARNING: Error calculation failed!")
    ...
    def autofit(self,electric_delay=None,fcrop=None,refine_results=False):
        '''
        automatic calibration and fitting
        electric_delay: set the electric delay manually
        fcrop = (f1,f2) : crop the frequency range used for fitting
        ...
        self.z_data = self.do_normalization(self.f_data,self.z_data_raw,delay,amp_norm,alpha,A2,frcal)
        self.fitresults = self.circlefit(self.f_data[self._fid],self.z_data[self._fid],fr,Ql,refine_results=refine_results,calc_errors=True)
        self.fitresults['prefactor_a'] = amp_norm
        self.fitresults['prefactor_alpha'] = alpha
        self.fitresults['baseline_slope'] = A2
        self.fitresults['baseline_intercept'] = -A2*frcal
